        ...
        <div className="bento-grid">
          ...
          <div className="bento-col-8 flex flex-col gap-6">
            ...
            <div className="card">
              <div className="card-header">
                <h3 className="text-headline-sm" style={{ fontWeight: 700 }}>{t('dashboard.recent_tasks')}</h3>
              ...
              {recentTasks.length > 0 ? (
                <div className="table-container" style={{ border: 'none', borderRadius: 0 }}>
                  <table>
                    <thead>
                      <tr>
                        <th>{t('dashboard.th_task')}</th>
                        <th>{t('dashboard.th_category')}</th>
                        <th>{t('dashboard.th_budget')}</th>
                      ...
                      {recentTasks.slice(0, 5).map((task) => (
                        <tr key={task.id}>
                          ...
                          <td style={{ fontWeight: 600 }}>
                            {task.budget !== undefined ? formatCurrency(task.budget) : t('dashboard.open_budget')}
                          </td>
                          <td>
                ...
                <div className="empty-state">
                  ...
                  <h3 className="text-headline-sm" style={{ marginBottom: 'var(--space-2)' }}>{t('dashboard.empty_tasks_title')}</h3>
                  <p style={{ marginBottom: 'var(--space-4)' }}>{t('dashboard.empty_tasks_desc')}</p>
                  <Link to="/client/tasks/new">
                    <button className="btn btn-primary">
                      <Plus size={16} /> {t('dashboard.btn_post_first')}
                    </button>
                  </Link>
                </div>
              )}
            ...
            {activeTasks.length > 0 && (
              <div>
                <div className="section-label">{t('dashboard.title_active_work')}</div>
          ...
          <div className="bento-col-4 flex flex-col gap-6">
            ...
            <div className="bento-card flex flex-col gap-4">
              <div className="section-label" style={{ marginBottom: 0 }}>{t('dashboard.title_quick_actions')}</div>
              
              <Link to="/client/tasks/new" style={{ width: '100%' }}>
                <button className="quick-action-primary">
                  <span>{t('dashboard.btn_post_new_task')}</span>
                  <Plus size={20} />
                </button>
              </Link>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-2)' }}>
                <Link to="/client/offers">
                  <button className="btn btn-outlined w-full" style={{ padding: '12px var(--space-2)', display: 'flex', flexDirection: 'column', gap: '6px', height: '100%', alignItems: 'center', justifyContent: 'center' }}>
                    <FileText size={18} />
                    <span style={{ fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.04em' }}>{t('tasks.offers')}</span>
                  </button>
                </Link>
                <Link to={`/profile/${currentUser?.id}`}>
                  <button className="btn btn-outlined w-full" style={{ padding: '12px var(--space-2)', display: 'flex', flexDirection: 'column', gap: '6px', height: '100%', alignItems: 'center', justifyContent: 'center' }}>
                    <User size={18} />
                    <span style={{ fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.04em' }}>{t('nav.profile')}</span>
                  </button>
                </Link>
              </div>
            </div>
              ...
              <div className="card-body" style={{ padding: 'var(--space-4)', display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
                {walletReserved > 0 && (
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: 'var(--space-3)', background: 'var(--color-status-warning-bg)', borderRadius: 'var(--radius)', border: '1px solid var(--color-outline-variant)' }}>
                    <div>
                      <div style={{ fontSize: 'var(--text-body-sm)', fontWeight: 600, color: 'var(--color-status-warning)' }}>{t('dashboard.label_escrow_hold')}</div>
                      <div style={{ fontSize: 'var(--text-label-md)', color: 'var(--color-on-surface-variant)' }}>{t('dashboard.label_escrow_hold_desc')}</div>
                    </div>
                        ...
                        <div key={tx.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 'var(--text-body-sm)' }}>
                          <div style={{ minWidth: 0 }}>
                            ...
                            <div style={{ fontSize: 'var(--text-label-md)', color: 'var(--color-on-surface-variant)' }}>
                              {tx.status === 'reserved' ? t('dashboard.tx_reserved') : tx.status === 'released' ? t('dashboard.tx_released') : t('dashboard.tx_refunded')}
                            </div>
                          </div>
